...
DenseNet121 emerged as the most effective architecture, achieving the highest validation accuracy and a test
accuracy of `96.98\%`. The additional time needed for including data augmentation in the training process did not
correspond to substantial accuracy gains, suggesting that augmentation's utility should be weighed against its computational
cost. To further confirm our hypotheses, a deeper analysis of `model\_densenet3` will be conducted using its
classification report and confusion matrix [Fig.10].
